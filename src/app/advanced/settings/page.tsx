...
const SIZE_PRESETS = [
  ...
  { label: 'M',    width: 80,  height: 80  },
  { label: 'L',    width: 120, height: 120 },
  { label: 'XL',   width: 160, height: 160 },
  ...
  async function handleGenerate() {
    ...
    try {
      const patternSettings = {
        ...
        stitchStyle:     settings.stitchStyle,
      ...
      <section style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '24px 20px 140px', gap: 16 }}>
        ...
        <div style={{ width: '100%', maxWidth: 400, background: 'white', borderRadius: 16, padding: '16px', boxShadow: '0 1px 6px rgba(44,34,24,0.06)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 8, alignItems: 'center' }}>
            <div>
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: '#9A8878', marginBottom: 4 }}>Width (stitches)</p>
              <input
            ...
            <div>
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: '#9A8878', marginBottom: 4 }}>Height (rows)</p>
              <input
          ...
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: '#C8BFB0', marginTop: 8 }}>
            10–300 stitches each direction · {settings.width * settings.height} total cells
          </p>
        </div>
        ...
        <div style={{ width: '100%', maxWidth: 400, background: 'white', borderRadius: 16, padding: '16px', boxShadow: '0 1px 6px rgba(44,34,24,0.06)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {(Object.keys(STITCH_STYLE_META) as StitchStyle[]).map(style => {
              ...
              const active = settings.stitchStyle === style
              return (
                <button
                  key={style}
                ...
                >
                  <span style={{ fontSize: 22, display: 'block', marginBottom: 4 }}>{STITCH_ICONS[style]}</span>
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 12, color: active ? '#C4614A' : '#2C2218' }}>{meta.label}</p>
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, color: '#9A8878', lineHeight: 1.3 }}>{meta.description}</p>
                </button>
              )
            })}
          </div>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'linear-gradient(to bottom, transparent, #FAF6EF 35%)', padding: '20px 20px max(24px, env(safe-area-inset-bottom))' }}>
        <div style={{ maxWidth: 400, margin: '0 auto' }}>
          ...
          >
            {isGenerating ? (
              <>
                <span style={{ width: 18, height: 18, borderRadius: '50%', border: '2.5px solid rgba(255,255,255,0.4)', borderTopColor: 'white', animation: 'adv-spin 0.8s linear infinite', display: 'inline-block' }} />
                Generating…
              </>
            ) : (
